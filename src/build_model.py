...
class TrainCustomModel:
    def __init__(self) -> None:
        ...
        self.MLFLOW_TRACKING_URI = os.environ.get('MLFLOW_TRACKING_URI', 'file:///tmp/mlflow-runs')
    ...
    def train_model(self, model_type: str):
        ...
        if model_type not in self.AVAILABLE_MODELS:
            logging.error(f"Invalid model_type '{model_type}'. Available options are: {list(self.AVAILABLE_MODELS.keys())}")
            return

        feature_file = os.path.join(self.FEATURES_PATH, 'customer_churn_20250821_155642.csv')
